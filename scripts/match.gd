...
func _ready() -> void:
	...
	# Give the ball to the first attacker at kickoff
	await get_tree().process_frame
	var first: Node2D = _pick_starting_carrier()
	if first:
		_give_ball_to(first)
...
func _pick_starting_carrier() -> Node2D:
	# Just grab the first attacker in the group
